...
const getFoodByNama = async (req, res) => {
...
const getFoodByNamaV2 = async (req, res) => {
  const nama = req.params.nama;
...
const updateFood = async (req, res) => {
  ...
  if (req.file) {
    ...
    if (food.gambar_lokasi) {
      const gambar_lokasi_old = food.gambar_lokasi.replace(
...
const deleteFood = async (req, res) => {
  ...
  if (food.Food_gambars.length !== 0) {
    ...
    for (let food_gambar of food_gambars) {
      const gambar_old = food_gambar.gambar.replace(
        `https://storage.googleapis.com/${process.env.GCS_BUCKET}/`,
        ""
      );
